...
const fetch_skyblock_profiles = async (uuid, apiKey = process.env.HYPIXEL_API_KEY) => {
    ...
    try {
        const response = await fetch(
            `https://api.hypixel.net/v2/skyblock/profiles?key=${apiKey}&uuid=${normalizedUuid}`,
            {
                method: 'GET',
                headers: {
...
const get_vanguard_corpse_count = async uuid => {
    ...
    const selectedProfileResult = select_preferred_skyblock_profile(profiles, normalizedUuid);
    if (!selectedProfileResult) {
        return {
            ok: false,
            failureCode: 'MISSING_PROFILE',
            message: 'No matching SkyBlock profile/member data was found for this player.',
            retryable: false,
        };
    }
